...
gcovr --root=. --filter=src/ml_flashpoint --exclude=".*/_deps/.*" --exclude="/tmp/.*" --exclude=".*/site-packages/.*" --gcov-executable=gcov --txt-metric branch --html-details htmlcov/cpp/index.html --xml-pretty -o cxx-coverage.xml --sort uncovered-number --gcov-ignore-parse-errors=negative_hits.warn,suspicious_hits.warn
