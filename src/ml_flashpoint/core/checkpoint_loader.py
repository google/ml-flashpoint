...
class DefaultMLFlashpointCheckpointLoader(MLFlashpointCheckpointLoader):
    ...
    def __init__(
        ...
        all_gather_object_func: Callable[..., None] = dist.all_gather_object,
        get_world_size_func: Callable[[], int] = dist.get_world_size,
    ):
        ...
        Args:
            ...
            global_rank_getter: A callable that returns the global rank. Defaults to
                ``torch.distributed.get_rank``.
            local_rank_getter: A callable that returns the node-local rank. Defaults to
                ``torch.distributed.get_node_local_rank``.
            broadcast_object_list_func: A callable with the same signature as
                ``torch.distributed.broadcast_object_list``. Defaults to that function.
            all_gather_object_func: A callable with the same signature as
                ``torch.distributed.all_gather_object``. Defaults to that function.
            get_world_size_func: A callable that returns the world size. Defaults to
        ...
        self._get_world_size_func = get_world_size_func
    ...
    def _compute_retrieval_plan(
        ...
        world_size = self._get_world_size_func()
    ...
    def get_candidate_checkpoints(
        ...
        all_checkpoint_container_path_lists = [None for _ in range(self._get_world_size_func())]
    ...
    def get_checkpoint_objects_by_rank(
        ...
        all_objects_by_rank_paths = [None for _ in range(self._get_world_size_func())]
    ...
    def retrieve_checkpoint(
        ...
        all_success_list = [None for _ in range(self._get_world_size_func())]
