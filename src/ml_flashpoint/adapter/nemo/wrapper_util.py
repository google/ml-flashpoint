...
def wrap_trainer_checkpoint_io_with_mlflashpoint(
    ...
    # Create the Manager in a background thread to avoid blocking the main trainer initialization.
    executor = concurrent.futures.ThreadPoolExecutor(max_workers=1)
    mp_manager_future = executor.submit(ctx.Manager)
